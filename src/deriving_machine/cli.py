...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Deriving-Machine thinks for you about the meanings.",
        epilog="Example:  deriving-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | deriving-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | deriving-machine         #

    $ deriving-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="deriving-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"deriving-machine: {utterance}.")
        syslog.closelog()
